...
#define AIR_SHADOW_THREAD_STACK_SIZE 	6144
...
int tc_air_shadow_example(int argc, char **argv)
{
	...
	if (2 == argc)
	{
		if (!strcmp("start", argv[1]))
		{
			if (1 == running_state)
			{
				Log_d("tc_air_shadow_example is already running\n");
				return 0;
			}			
		}
		else if (!strcmp("stop", argv[1]))
		{
			if (0 == running_state)
			{
				Log_d("tc_air_shadow_example is already stopped\n");
				return 0;
			}
			running_state = 0;
			return 0;
		...
		{
			Log_d("Usage: tc_air_shadow_example start/stop");
			return 0;			  
		}
	}
	else
	{
		Log_e("Para err, usage: tc_air_shadow_example start/stop");
		return 0;
	}
...
#include <finsh.h>
FINSH_FUNCTION_EXPORT(tc_air_shadow_example, startup air shadow example);
#endif

#ifdef FINSH_USING_MSH
MSH_CMD_EXPORT(tc_air_shadow_example, startup air shadow example);
#endif
